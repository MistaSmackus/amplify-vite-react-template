...
import { createWatchStock } from "../graphql/mutations";
const client = generateClient();
export default function WatchStockCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setChange(initialValues.change);
    setErrors({});
  };
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: createWatchStock.replaceAll("__typename", ""),
            variables: {
              input: {
                ...modelFields,
              },
      ...
      {...getOverrideProps(overrides, "WatchStockCreateForm")}
      {...rest}
    >
      <TextField
        label="Symbol"
